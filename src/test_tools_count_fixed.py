#!/usr/bin/env python3
"""
Test script to enumerate all available MCP tools in ProxmoxMCP without requiring Proxmox connection.
"""
import os
import sys
import asyncio
...
async def test_mcp_tools_mock():
    """Test and enumerate all registered MCP tools with mocked Proxmox connection."""
    try:
        ...
        with patch('proxmox_mcp.core.node_manager.ProxmoxManager') as mock_manager:
            with patch('proxmoxer.ProxmoxAPI') as mock_api:
                ...
                print("🚀 Initializing ProxmoxMCP Server (with mocked connection)...")
...
def main():
    ...
    print("🔍 ProxmoxMCP Tool Enumeration (Mock Mode)")
    print("=" * 50)
    
    # Run the async test
    result = asyncio.run(test_mcp_tools_mock())
